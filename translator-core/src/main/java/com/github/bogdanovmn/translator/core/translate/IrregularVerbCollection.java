...
public class IrregularVerbCollection {
	private final Map<String, IrregularVerb> verbMap;

	private IrregularVerbCollection(Map<String, IrregularVerb> verbMap) {
		this.verbMap = verbMap;
	}

	public static IrregularVerbCollection create() {
		Map<String, IrregularVerb> verbMap = new HashMap<>();
		for (String[] forms: FORMS) {
			String translate = forms[3];
			IrregularVerb irrVerb = new IrregularVerb(
				forms[0], forms[1], forms[2], translate
			);
			for (int i = 0; i < 3; i++) {
				String[] formVariants = forms[i].split("\\W+");
				for (String formVariant : formVariants) {
					verbMap.put(formVariant, irrVerb);
				}
			}
		}
		return new IrregularVerbCollection(verbMap);
	}

	public Optional<IrregularVerb> get(String word) {
